    ...
    private final class Entry {
        ...
        private Entry(@NotNull List<PropInfo> propInfos) throws RepositoryException {
            ...
            for (PropInfo prop : propInfos) {
                ...
                if (REP_EFFECTIVE_PATH.equals(oakName)) {
                    effectivePath = prop.getTextValue().getString();
                } else if (REP_PRIVILEGES.equals(oakName)) {
                    privs = getPrivileges(Iterables.transform(prop.getTextValues(), textValue -> textValue.getString()));
                } else {
                    throw new ConstraintViolationException("Unsupported property '"+oakName+"' with policy entry of type rep:PrincipalEntry");
                }
            }
            if (privs == null) {
